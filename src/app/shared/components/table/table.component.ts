...
export class TableComponent implements OnDestroy {
  ...
  getBadgeClass(value: any): string {
    if (value === null || value === undefined) return 'p-badge-secondary';
    
    // Handle booleans explicitly
    if (typeof value === 'boolean') {
      return value ? 'p-badge-success' : 'p-badge-warning';
    }

    const val = String(value).toUpperCase();
    if (val === 'ORIGINAL') return 'p-badge-secondary p-badge-outlined';
    if (val === 'AMENDMENT') return 'p-badge-info';
    if (val.includes('NOT_FOUND') || val.includes('ERROR') || val.includes('FAILED')) return 'p-badge-danger';
    if (val === 'COMPLETED' || val === 'SUCCESS' || val === 'APPROVED') return 'p-badge-success';
    if (val === 'PROCESSING' || val === 'QUEUED' || val === 'PENDING') return 'p-badge-warning';
